...
//						"select id, name, email,country from Users where email=? and password=? limit 1");
				ps = con.prepareStatement(
						"select id, name, email,country from Users where email=? and password=? FETCH FIRST 1 ROWS ONLY");
				...
				throw new ServletException("MyMessage" + e.getMessage() + "DB Connection problem.");
			} 
//			finally {
//				try {
//					rs.close();
